...
type Member struct {
	...
	TimesMentioned  int       // Number of times a user, other than self mentioned them
}
...
type GuildStats struct {
	...
	AttachmentsSent int // Number of attachments sent
	LinksSent       int // Number of links sent
	Mentions        int // Number of mentions made
}
type Guild struct {
...
func GetGuildStats(id string) GuildStats {
	var guild Guild
	err = db.One("ID", id, &guild)
	if err != nil {
		logger.Error(err)
...
func AddGuild(g *discordgo.Guild) error {
	var guildCheck Guild
	err = db.One("ID", g.ID, &guildCheck)
	if err == nil {
		return fmt.Errorf("[%v], %v", g.ID, "Guild already exists.")
	} else {
		...
		for _, member := range g.Members {
			...
			GlobalStatsCount = GlobalStats{
				GlobalStatsCount.Members + 1,
				GlobalStatsCount.Messages,
				GlobalStatsCount.Mentions,
				GlobalStatsCount.Links,
				GlobalStatsCount.Attachments,
				GlobalStatsCount.Guilds,
			}
		}
		...
		GlobalStatsCount = GlobalStats{
			GlobalStatsCount.Members,
			GlobalStatsCount.Messages,
			GlobalStatsCount.Mentions,
			GlobalStatsCount.Links,
			GlobalStatsCount.Attachments,
			GlobalStatsCount.Guilds + 1,
		}
...
func AddMessage(m *discordgo.MessageCreate) error {
	...
	if len(m.Mentions) > 0 {
		for _, v := range m.Mentions {
			...
			guild.Data.Members[v.ID] = Member{
				guild.Data.Members[v.ID].GuildID,
				guild.Data.Members[v.ID].JoinedAt,
				guild.Data.Members[v.ID].Nick,
				guild.Data.Members[v.ID].User,
				guild.Data.Members[v.ID].Roles,
				guild.Data.Members[v.ID].MessagesSent,
				guild.Data.Members[v.ID].AttachmentsSent,
				guild.Data.Members[v.ID].LinksSent,
				guild.Data.Members[v.ID].TimesMentioned + 1,
			}
			guild.Stats = GuildStats{
				guild.Stats.TotalMessages,
				guild.Stats.MessagesTM,
				guild.Stats.MessagesTD,
				guild.Stats.MessagesTH,
				guild.Stats.MessagesTm,

				guild.Stats.TotalMembers,
				guild.Stats.MembersTM,
				guild.Stats.MembersTD,
				guild.Stats.MembersTH,
				guild.Stats.MembersTm,

				guild.Stats.AttachmentsSent,
				guild.Stats.LinksSent,
				guild.Stats.Mentions + 1,
			}
			GlobalStatsCount = GlobalStats{
				GlobalStatsCount.Members,
				GlobalStatsCount.Messages,
				GlobalStatsCount.Mentions + 1,
				GlobalStatsCount.Links,
				GlobalStatsCount.Attachments,
				GlobalStatsCount.Guilds,
			}
	...
	guild.Data.Members[m.Author.ID] = Member{
		...
		guild.Data.Members[m.Author.ID].TimesMentioned,
	}
	guild.Stats = GuildStats{
		guild.Stats.TotalMessages + 1,
		guild.Stats.MessagesTM,
		guild.Stats.MessagesTD,
		guild.Stats.MessagesTH,
		guild.Stats.MessagesTm,
		...
		guild.Stats.AttachmentsSent,
		guild.Stats.LinksSent,
		guild.Stats.Mentions,
	}
	GlobalStatsCount = GlobalStats{
		GlobalStatsCount.Members,
		GlobalStatsCount.Messages + 1,
		GlobalStatsCount.Mentions,
		GlobalStatsCount.Links,
		GlobalStatsCount.Attachments,
		GlobalStatsCount.Guilds,
	...
	if len(message.Attachments) > 0 {
		...
		guild.Stats = GuildStats{
			...
			guild.Stats.AttachmentsSent + len(message.Attachments),
			guild.Stats.LinksSent,
			guild.Stats.Mentions,
		}
		GlobalStatsCount = GlobalStats{
			...
			GlobalStatsCount.Mentions,
			GlobalStatsCount.Links,
			GlobalStatsCount.Attachments + len(message.Attachments),
			GlobalStatsCount.Guilds,
		}
	}
	...
	if len(xurls.Relaxed().FindAllString(m.Content, -1)) > 0 {
		...
		guild.Stats = GuildStats{
			...
			guild.Stats.AttachmentsSent,
			guild.Stats.LinksSent + len(xurls.Relaxed().FindAllString(m.Content, -1)),
			guild.Stats.Mentions,
		}
		GlobalStatsCount = GlobalStats{
			GlobalStatsCount.Members,
			GlobalStatsCount.Messages,
			GlobalStatsCount.Mentions,
			GlobalStatsCount.Links + len(xurls.Relaxed().FindAllString(m.Content, -1)),
			GlobalStatsCount.Attachments,
			GlobalStatsCount.Guilds,
		}
	}
...
func CheckUpToDate(g *discordgo.Guild) bool {
	stats := InitStats()
	GlobalStatsCount = GlobalStats{
		stats["members"],
		stats["messages"],
	...
	neededEdit = false
	guilds := GetGuilds()
	if !containsGuild(guilds, g) {
		AddGuild(g)
		neededEdit = true
	}
	// Re-populate with new
	guildlocal := GetGuild(g.ID)
	...
	if g.MemberCount != guildlocal.Stats.TotalMembers {
		guildlocal.Stats = GuildStats{
			guildlocal.Stats.TotalMessages,
			guildlocal.Stats.MessagesTM,
			guildlocal.Stats.MessagesTD,
			guildlocal.Stats.MessagesTH,
			guildlocal.Stats.MessagesTm,

			g.MemberCount,
			guildlocal.Stats.MembersTM,
			guildlocal.Stats.MembersTD,
			guildlocal.Stats.MembersTH,
			guildlocal.Stats.MembersTm,

			guildlocal.Stats.AttachmentsSent,
			guildlocal.Stats.LinksSent,
			guildlocal.Stats.Mentions,
		}
		err := db.Save(&guildlocal)
		if err != nil {
			logger.Error(err)
		}
		neededEdit = true
...
func InitStats() map[string]int {
	...
	for _, guild := range guilds {
		members += guild.Stats.TotalMembers
		messages += guild.Stats.TotalMessages
		mentions += guild.Stats.Mentions
		links += guild.Stats.LinksSent
		attachments += guild.Stats.AttachmentsSent
	}
...
func GetStats() map[string]int {
	...
	stats["members"] = GlobalStatsCount.Members
	stats["messages"] = GlobalStatsCount.Messages
	stats["guilds"] = GlobalStatsCount.Guilds
	stats["mentions"] = GlobalStatsCount.Mentions
	stats["links"] = GlobalStatsCount.Links
	stats["attachments"] = GlobalStatsCount.Attachments
